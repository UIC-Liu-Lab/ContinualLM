...
def compute_impt(args,config, model,eval_dataloader,accelerator,prune_loss=None):
    ...
    for step, inputs in enumerate(tqdm(eval_dataloader, desc="Iteration " + prune_loss)):
        ...
        # print('head_mask.grad: ',head_mask.grad)
    ...
    if accelerator.is_main_process:
        np.save(os.path.join(args.output_dir, prune_loss + str(args.pt_task)+"/head_impt.npy"), head_impt.detach().cpu().numpy())
        np.save(os.path.join(args.output_dir, prune_loss +  str(args.pt_task)+"/intermediate_impt.npy"),intermediate_impt.detach().cpu().numpy())
        np.save(os.path.join(args.output_dir, prune_loss +  str(args.pt_task)+"/output_impt.npy"), output_impt.detach().cpu().numpy())
...
def accumulate_impt(args):
    ...
    for impt_dir_id, impt_dir in enumerate(args.saved_output_dir):
        # head
        if not path.exists(os.path.join(impt_dir, "head_impt.npy")): continue
        print('os.path.join(impt_dir, "head_impt.npy"): ', os.path.join(impt_dir, "head_impt.npy"))

        head_impt = torch.Tensor(np.load(os.path.join(impt_dir, "head_impt.npy"))).cuda()
        head_impt = impt_norm(head_impt)
        head_impt_list.append(head_impt)

        # intermediate
        intermediate_impt = torch.Tensor(np.load(os.path.join(impt_dir, "intermediate_impt.npy"))).cuda()
        intermediate_impt = impt_norm(intermediate_impt)
        intermediate_impt_list.append(intermediate_impt)

        output_impt = torch.Tensor(np.load(os.path.join(impt_dir, "output_impt.npy"))).cuda()
